...
void commit_set_action(struct ofpbuf *odp_actions, enum ovs_key_attr key_type,
...
static void *
get_ofp_port_pof(const struct xbridge *xbridge, ofp_port_t *ofp_port, odp_port_t *odp_port, int *cnt)
{
    struct xport *xport;
    ...
    for(i = 0; i < 32; i++) {
        ofp_port[i] = i + 1;
        odp_port[i] = OFPP_METADATA;
        VLOG_INFO("+++++++ pjq ofp_port[%d] = :%d\n", i, ofp_port[i]);

        HMAP_FOR_EACH_IN_BUCKET (xport, ofp_node, hash_ofp_port(ofp_port[i]),
                                 &xbridge->xports) {
            if (xport->ofp_port == ofp_port[i]) {
                odp_port[i] = xport->odp_port;
                VLOG_INFO("+++++++ pjq ofp_port[%d] = :%d, odp_port[%d] = :%d\n", i, ofp_port[i], i, odp_port[i]);
                *cnt = i + 1;
                VLOG_INFO("++++ pjq i:%d, *cnt:%d", i, *cnt);
                break;
            }
...
{
    ofp_port_t ofp[PORT_NUM];
    odp_port_t odp[PORT_NUM];
    int port_map_num = 0;
    const struct xport *xport;
    VLOG_INFO("+++++ pjq  ofp_port:%d, OFPP_METADATA:%d", ofp_port, OFPP_METADATA);
    if (ofp_port == OFPP_METADATA) {
        int cnt = 0;
        VLOG_INFO("++++ pjq come in");
        get_ofp_port_pof(ctx->xbridge, ofp, odp, &cnt);
        VLOG_INFO("+++pjq cnt:%d", cnt);
        port_map_num = cnt;
        for(int i = 0; i < cnt; i++) {
            VLOG_INFO("+++++++ pjq ofp_port[%d] = :%d, odp_port[%d] = :%d\n", i, ofp[i], i, odp[i]);
        }
        ...
        VLOG_INFO("++++++ pjq xport->odp_port:%d", xport->odp_port);
    }
    ...
    if (xport->is_tunnel) {          //*********sqy notes: xport->is_tunnel = false, no run
        ...
        VLOG_INFO("+++++ pjq out_port:%d, ofp_port:%d", out_port, ofp_port);
    }


ofpp_metadata:
    if (out_port == OFPP_METADATA) {
        odp_port = xport->odp_port;
        out_port = odp_port;
    }
    VLOG_INFO("+++++ pjq run here 4");
            ...
            if (tnl_push_pop_send) {
                ...
                * int tunnel-port flow to avoid these checks completely. */
                ...
                if (odp_tnl_port != ODPP_NONE) {
                    ...
                    VLOG_INFO("+++++ pjq before nl_msg_put_odp_port");
                    struct pof_flow *flow = &ctx->xin->flow;

                    struct ovs_key_output key;
                    key.portId_type = (uint8_t)flow->pad_to_flow[0][0];
                    if(key.portId_type == 1) {
                        key.port = (uint32_t)OFPP_METADATA;
                        key.offset = flow->pad_to_flow[0][2];
                        key.len = flow->pad_to_flow[0][3];
                        key.port_cnt = port_map_num;
                        for(int i = 0; i < port_map_num; i++) {
                            key.odp_map[i] = odp[i];
                        }

                        VLOG_INFO("+++++ pjq portId_type:%d, port:%d, offset:%d, len:%d, sizeof key:%d, cnt:%d",
                                   key.portId_type, key.port, key.offset, key.len, sizeof key, port_map_num);

                        struct ofpbuf *msg = ctx->odp_actions;
                        nl_msg_put_unspec(msg, OVS_ACTION_ATTR_OUTPUT, &key, sizeof key);

                        struct ds s;
                        ds_init(&s);
                        ds_put_hex_dump(&s, msg->data - sizeof key, sizeof key, 0, false);
                        VLOG_INFO("+++++ pjq msg->data:\n%s", ds_cstr(&s));
                        ds_destroy(&s);


                        ds_init(&s);
                        ds_put_hex_dump(&s, &key, sizeof key, 0, false);
                        VLOG_INFO("+++++ pjq key:\n%s", ds_cstr(&s));
                        ds_destroy(&s);
                    } else {
                        VLOG_INFO("+++++ pjq out_port:%d", out_port);
                        nl_msg_put_odp_port(ctx->odp_actions,
                 ...
                 struct xlate_ctx *ctx)
{
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_OUTPUT:
        	...
        	flow->pad_to_flow[0][0] = output->portId_type;
        	flow->pad_to_flow[0][1] = output->field_id;
        	flow->pad_to_flow[0][2] = output->offset;
        	flow->pad_to_flow[0][3] = output->len;

        	VLOG_INFO("+++++ pjq port:%d, portId_type:%d, field_id:%d, offset:%d, len:%d",
        	           output->port, output->portId_type, output->field_id, output->offset, output->len);
...
//        	wc->masks.have_group_action = true;
       	...
       	case OFPACT_WRITE_METADATA_FROM_PACKET: {              /* pjq */
            ...
            //VLOG_INFO("++++++++ pjq metadata_offset:%d, packet_offset:%d, field_len:%d",
             //         write_metadata_from_packet->metadata_offset, write_metadata_from_packet->packet_offset,
             //         write_metadata_from_packet->field_len);
        ...
        if (!ctx->error && ctx->exit && ctx_first_frozen_action(ctx)) {
            //VLOG_INFO("++++++ pjq before freeze_unroll_actions");
