                          ...
                          struct ofpbuf *out)
{
    ...
    output->portId_type = oao->portId_type;

    output->max_len = output->port == OFPP_CONTROLLER ? UINT16_MAX : 0;



    if(oao->portId_type == 1) {
        output->field_id = ntohs(oao->field.field_id);
        output->offset = ntohs(oao->field.offset);
        output->len = ntohs(oao->field.len);
        output->port = OFPP_METADATA;
        VLOG_INFO("+++++++++++pjq decode_OFPAT_RAW11_OUTPUT:port(from metadata)=%"PRIu16", max_len=%"PRIu16,
              ...
              enum ofp_version ofp_version, struct ofpbuf *out)
{
    if (ofp_version == OFP10_VERSION) {
        ...
        oao->portId_type = output->portId_type;
        if(output->portId_type == 1) {
            oao->field.field_id = htons(output->field_id);
            oao->field.offset = htons(output->offset);
            oao->field.len = htons(output->len);
        } else {
...
format_OUTPUT(const struct ofpact_output *a, struct ds *s)
{
    if (ofp_to_u16(a->port) < ofp_to_u16(OFPP_MAX)) {
        ...
        if (a->port == OFPP_METADATA) {
            VLOG_INFO("port is from metadata");
        }
    } else {
               ...
               enum ofp_version ofp_version, struct ofpbuf *ofpacts)
{
    ...
    for (i=0; i<actions_len/POF_MAX_ACTION_LENGTH; i++){
        ...
        if (!error) {
            ...
            ds_init(&s);
            ds_put_hex_dump(&s, actions, actions_len, 0, false);
            VLOG_INFO("+++++tsf ofpacts_decode: ds_put_hex_dump_action:\n %s", ds_cstr(&s));
            ds_destroy(&s);
