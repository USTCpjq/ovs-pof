...
//struct xbridge *xbridge_lookup(struct xlate_cfg *xcfg, const struct ofproto_dpif *ofproto);
//
//
// struct ofproto_dpif *xlate_lookup_ofproto_(const struct dpif_backer *backer, const struct flow *flow,
//                      ofp_port_t *ofp_in_port, const struct xport **xportp);
//
// struct xport *get_ofp_port(const struct xbridge *xbridge, ofp_port_t ofp_port);
//
//
//
                     ...
                     int *lost_cnt, long long now)
{
   ...
   // VLOG_INFO("++++++ pjq the addr of b:%p", &b);
     ...
     struct ds s;
     ds_init(&s);
     ds_put_hex_dump(&s, actions->data, actions->size , 0, false);
     VLOG_INFO("+++++ pjq actions->data:\n%s", ds_cstr(&s));
     ds_destroy(&s);
...
#define get_mask(a, type) ((const type *)(const void *)(a + 2))
              ...
              struct pofdp_metadata_batch *pofdp_mds)
{
    ...
    VLOG_INFO("+++++ pjq type:%d, OVS_ACTION_ATTR_OUTPUT:%d", type, OVS_ACTION_ATTR_OUTPUT);
    long long now = aux->now;
    struct tx_port *p;

    struct ovs_key_output *oko, *key;

    struct ds s;
    ds_init(&s);
    ds_put_hex_dump(&s, a, 32, 0, false);
...
//        p = pmd_tx_port_cache_lookup(pmd, u32_to_odp(nl_attr_get_u32(a)));
        ...
        oko = get_mask(a, struct ovs_key_output);

        VLOG_INFO("++++ pjq oko addr:%p,  %p, %p, %p, %p, a addr:%p, %p, %p, %p", oko-2, oko-1, oko, oko+1, oko+2, a-1, a, a+1, a+2);

        oko = (void *)oko;

        VLOG_INFO("++++ pjq oko addr:%p,  %p, %p, %p, %p", (void *)oko-2, (void *)oko-1, (void *)oko, (void *)oko+1, (void *)oko+2);

        oko = (uint8_t *)oko;
        VLOG_INFO("++++ pjq oko addr:%p,  %p, %p, %p, %p", (uint8_t *)oko-2, (uint8_t *)oko-1, (uint8_t *)oko, (uint8_t *)oko+1, (uint8_t *)oko+2);


        key = (uint8_t *)oko-4;

        VLOG_INFO("++++ pjq portId_type:%d, offset:%d, len:%d, port:%d", key->portId_type, key->offset, key->len, key->port);


        struct ds s;
        ds_init(&s);
        ds_put_hex_dump(&s, key, 24, 0, false);
        VLOG_INFO("+++++ pjq key:\n%s", ds_cstr(&s));
        ds_destroy(&s);
        ...
        VLOG_INFO("++++ pjq portId_type:%d, offset:%d, len:%d, port:%d, cnt:%d",
                oko->portId_type, oko->offset, oko->len, oko->port, oko->port_cnt);

        for(int i = 0; i < oko->port_cnt; i++) {
            VLOG_INFO("+++++ pjq odp_map[%d]: %d", i, oko->odp_map[i]);
        }


        uint16_t offset = oko->offset;
        uint16_t len = oko->len;
...
//            struct udpif *udpif = dp->upcall_aux;
//            udpif = (struct udpif *)malloc(sizeof(struct udpif));
//            struct ofproto_dpif *ofproto;
//            struct xport *xport;
//
//            ofproto = xlate_lookup_ofproto_(udpif->backer, aux->flow, aux->flow->in_port.ofp_port, &xport);
...
//
            ...
            uint32_t value = 0;
            pof_mds[0] = (struct pofdp_metadata *)malloc(sizeof(struct pofdp_metadata));

            struct ds s;
            ds_init(&s);
            ds_put_hex_dump(&s, pof_mds[0], 32, 0, false);
            VLOG_INFO("++++++ pjq pof_mds[0]:\n%s", ds_cstr(&s));
            ds_destroy(&s);



            pofbf_copy_bit((uint8_t *)pof_mds[0], (uint8_t  *)&value, offset, PORT_LEN);




            VLOG_INFO("++++++++ pjq port value:%d", value);
            value = ntohl(value);
            VLOG_INFO("++++++++ pjq port value:%d", value);
            value = POF_MOVE_BIT_RIGHT(value, PORT_LEN - len);
            VLOG_INFO("++++++++ pjq port value:%d", value);
...
//            xport = (struct xport *)malloc(sizeof(struct xport));
//            xport = get_ofp_port(xbridge, value);
//
//            value = xport->odp_port;

            VLOG_INFO("++++++++ pjq port value:%d", value);


            value = oko->odp_map[value - 1];

            VLOG_INFO("++++++++ pjq port value:%d", value);

            //p->port->port_no = ntohl(1);
            p = pmd_tx_port_cache_lookup(pmd, u32_to_odp((value)));
            ...
            p = pmd_tx_port_cache_lookup(pmd, u32_to_odp(nl_attr_get_u32(a)));
            VLOG_INFO("++++ pjq nl_attr_get_u32(a):%d, u32_to_odp:%d", nl_attr_get_u32(a), u32_to_odp(nl_attr_get_u32(a)));
        }
